...
fig, axs = plot_fit_params(exper_fits, label='Experiment', colour='#f89540ff')

# Add the second set of plots to the same figure
fig, axs = plot_fit_params(
    RMT_fits, colour='#0d0887ff', label='RMT', fig=fig, axs=axs)

# Add another set if needed
fig, axs = plot_fit_params(
    RMT_detune_fits, colour='#cc4778ff', label='RMT, detuned', fig=fig, axs=axs)
